...
class OCTAbaqusUtilities(ThermalElasticAbaqusUtilities, HeatConductionAbaqusUtilities):
    ...
    def create_model(model_name: str, geo_params_scaled: Dict[str, float], mesh_params_scaled: Dict[str, float]):
        ...
        outer_radius_xy, outer_radius_yz, outer_radius_zx = (geo_params_scaled[k] for k in ['outer_radius_xy', 'outer_radius_yz', 'outer_radius_zx'])
        inner_radius_xy, inner_radius_yz, inner_radius_zx = (geo_params_scaled[k] for k in ['inner_radius_xy', 'inner_radius_yz', 'inner_radius_zx'])
        ...
        target_vector = np.array([xDimension, yDimension, 0])
        ...
        target_vector = np.array([0, yDimension, zDimension])
        initial_vector = np.array([0, 0, zDimension]) 
        rotation_axis = np.cross(initial_vector, target_vector)
        rotation_axis = rotation_axis / np.linalg.norm(rotation_axis)
        rotation_angle = degrees(acos(np.dot(initial_vector, target_vector) / 
        ...
        target_vector = np.array([xDimension, 0, zDimension])
        initial_vector = np.array([0, 0, zDimension]) 
        rotation_axis = np.cross(initial_vector, target_vector)
        rotation_axis = rotation_axis / np.linalg.norm(rotation_axis)
        rotation_angle = degrees(acos(np.dot(initial_vector, target_vector) / 
        ...
        partMeshOCT = model.PartFromMeshMirror(name=part_name,part=partMeshOCT_1_2,
        point1=(0, yDimension, 0),
        point2=(-1, yDimension, 0))

        instance_name = AbaqusUtilities._get_instance_name(model_name)
        OCTInst = assembly.Instance(name=instance_name, part=partMeshOCT, dependent=ON)
